...
from utils.pipelines.td_pipeline import SimMaskedAdaptation
from utils.pipelines.masked_source import SourceAdaptation
...
def load_model(checkpoint_path, model,char):
    # reloads model
...
def adapt(config):
    ...
    if config.adaptation.student_checkpoint is not None:
        student_model = load_model(config.adaptation.student_checkpoint, student_model,'student_model.')
    ...
    if config.adaptation.teacher_checkpoint is not None:
        teacher_model = load_model(config.adaptation.teacher_checkpoint, teacher_model,'teacher_model.')
